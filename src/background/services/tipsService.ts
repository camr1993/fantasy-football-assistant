  ...
  try {
    ...
    if (result.success && result.data) {
      ...
      console.log('Tips data stored successfully', {
        ...
        leaguesWithRecommendations: Object.keys(playerRecommendations).length,
...
/**
 * Build a map of player recommendations indexed by league ID, then by Yahoo
 * player ID (numeric part only). Yahoo player IDs are stored as "461.p.40042"
 * but the DOM uses just "40042".
 *
 * Keying by league as well as player matters for anyone in more than one
 * league: the same player can be rostered in several of them, and a flat
 * player-keyed map would keep only whichever league was processed last.
 */
function buildPlayerRecommendationsMap(
  ...
  const entryFor = (leagueId: string, yahooId: string) => {
    if (!map[leagueId]) {
      map[leagueId] = {};
    }
    if (!map[leagueId][yahooId]) {
      map[leagueId][yahooId] = {};
    }
    return map[leagueId][yahooId];
  };
  ...
  if (tips.start_bench_recommendations) {
    for (const rec of tips.start_bench_recommendations) {
      ...
      if (!yahooId || !rec.league_id) continue;

      entryFor(rec.league_id, yahooId).startBench = rec;
    }
  }
  ...
  if (tips.waiver_wire_recommendations) {
    for (const rec of tips.waiver_wire_recommendations) {
      ...
      const entry = entryFor(rec.league_id, yahooId);
      if (!entry.waiverUpgrades) {
        entry.waiverUpgrades = [];
      }
      entry.waiverUpgrades.push(rec);
    }
  }
