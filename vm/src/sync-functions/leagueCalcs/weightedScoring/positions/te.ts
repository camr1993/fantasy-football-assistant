...
/**
 * Calculate weighted scores for all TE players in a league
 */
export async function calculateWeightedScoresForLeagueTE(
  leagueId: string,
  seasonYear: number,
  week: number
): Promise<void> {
  logger.info('Calculating weighted scores for TE players', {
    leagueId,
    seasonYear,
    week,
  });

  // Get all TE players who have fantasy points calculated for this week
  // Use !inner to force INNER JOIN so we only get players that exist and match the position filter
  const { data: tePlayers, error: fetchError } = await supabase
    .from('league_calcs')
    .select(
      `
      player_id,
  ...
  if (fetchError) {
    logger.error('Failed to fetch TE players for weighted score calculation', {
      error: fetchError,
      leagueId,
      seasonYear,
      week,
  ...
  if (!tePlayers || tePlayers.length === 0) {
    logger.info('No TE players found for weighted score calculation', {
      leagueId,
      seasonYear,
      week,
    });
  ...
  const playerIds = tePlayers.map((p: any) => p.player_id);

  logger.info('Fetched TE players for weighted score calculation', {
    leagueId,
    seasonYear,
    week,
  ...
  // 2. Get all efficiency metrics in one query
  logger.info('Fetching efficiency metrics for weighted scores', {
    leagueId,
    seasonYear,
    week,
  ...
  const { data: efficiencyMetrics, error: efficiencyError } = await supabase
    .from('player_stats')
    .select(
      'player_id, targets_per_game_3wk_avg_norm, yards_per_target_3wk_avg_norm, receiving_touchdowns_3wk_avg_norm'
    )
    .in('player_id', playerIds)
    .eq('season_year', seasonYear)
    .eq('week', week)
  ...
  if (efficiencyError) {
    logger.error('Failed to fetch efficiency metrics for weighted scores', {
  ...
  logger.info('Fetched efficiency metrics for weighted scores', {
    leagueId,
    seasonYear,
    week,
  ...
  if (defensePlayerIds.length > 0) {
    const result = await supabase
      ...
      .select('player_id, te_rolling_3_wk_avg_norm')
      .eq('league_id', leagueId)
      .eq('season_year', seasonYear)
      .eq('week', week)
  ...
  if (opponentDifficultyData) {
    for (const od of opponentDifficultyData) {
      difficultyMap.set(od.player_id, od.te_rolling_3_wk_avg_norm || 0);
    }
  }
  ...
  logger.info('Calculating weighted scores for all players', {
    leagueId,
    seasonYear,
    week,
    playerDataCount: playerData.length,
  ...
  for (const player of playerData) {
    ...
    const targetsPerGameNorm = efficiency?.targets_per_game_3wk_avg_norm || 0;
    const receivingTouchdownsNorm =
      efficiency?.receiving_touchdowns_3wk_avg_norm || 0;
    const yardsPerTargetNorm = efficiency?.yards_per_target_3wk_avg_norm || 0;
  ...
  logger.info('Completed weighted score calculation for TE players', {
    leagueId,
    seasonYear,
    week,
    tePlayersUpdated: updates.length,
  });
}
