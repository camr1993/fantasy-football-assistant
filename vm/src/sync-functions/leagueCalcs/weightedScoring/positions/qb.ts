...
/**
 * Calculate weighted scores for all QB players in a league
 */
export async function calculateWeightedScoresForLeagueQB(
  ...
  logger.info('Calculating weighted scores for QB players', {
    leagueId,
    seasonYear,
    week,
  });

  // Get all QB players who have fantasy points calculated for this week
  // Use !inner to force INNER JOIN so we only get players that exist and match the position filter
  const { data: qbPlayers, error: fetchError } = await supabase
    .from('league_calcs')
    .select(
      `
      player_id,
  ...
  if (fetchError) {
    logger.error('Failed to fetch QB players for weighted score calculation', {
      error: fetchError,
      leagueId,
      seasonYear,
      week,
  ...
  if (!qbPlayers || qbPlayers.length === 0) {
    logger.info('No QB players found for weighted score calculation', {
      leagueId,
      seasonYear,
      week,
    });
  ...
  const playerIds = qbPlayers.map((p: any) => p.player_id);

  logger.info('Fetched QB players for weighted score calculation', {
    leagueId,
    seasonYear,
    week,
  ...
  const { data: efficiencyMetrics, error: efficiencyError } = await supabase
    .from('player_stats')
    .select(
      'player_id, passing_efficiency_3wk_avg_norm, turnovers_3wk_avg_norm, rushing_upside_3wk_avg_norm'
    )
    .in('player_id', playerIds)
    .eq('season_year', seasonYear)
    .eq('week', week)
  ...
  if (defensePlayerIds.length > 0) {
    const result = await supabase
      ...
      .select('player_id, qb_rolling_3_wk_avg_norm')
      .eq('league_id', leagueId)
      .eq('season_year', seasonYear)
      .eq('week', week)
  ...
  if (opponentDifficultyData) {
    for (const od of opponentDifficultyData) {
      difficultyMap.set(od.player_id, od.qb_rolling_3_wk_avg_norm || 0);
    }
  }
  ...
  for (const player of playerData) {
    ...
    const passingEfficiencyNorm =
      efficiency?.passing_efficiency_3wk_avg_norm || 0;
    const turnoversNorm = efficiency?.turnovers_3wk_avg_norm || 0;
    const rushingUpsideNorm = efficiency?.rushing_upside_3wk_avg_norm || 0;
  ...
  logger.info('Completed weighted score calculation for QB players', {
    leagueId,
    seasonYear,
    week,
    qbPlayersUpdated: updates.length,
  });
}
