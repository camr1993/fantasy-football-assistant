...
export async function calculateWeightedScoresForLeagueWR(
  ...
  const playerIds = wrPlayers.map((p: any) => p.player_id);

  logger.info('Fetched WR players for weighted score calculation', {
    leagueId,
    seasonYear,
    week,
  ...
  const { data: efficiencyMetrics, error: efficiencyError } = await supabase
    .from('player_stats')
    .select(
      'player_id, targets_per_game_3wk_avg_norm, catch_rate_3wk_avg_norm, yards_per_target_3wk_avg_norm'
    )
    .in('player_id', playerIds)
    .eq('season_year', seasonYear)
    .eq('week', week)
  ...
  for (const player of playerData) {
    ...
    const catchRateNorm = efficiency?.catch_rate_3wk_avg_norm || 0;
    const yardsPerTargetNorm = efficiency?.yards_per_target_3wk_avg_norm || 0;

    // Calculate weighted score using the formula
    const weightedScore =
      POSITION_WEIGHTS.WR.recent_mean.weight * recentMean +
      POSITION_WEIGHTS.WR.volatility.weight * recentStd +
      POSITION_WEIGHTS.WR.targets_per_game.weight * targetsPerGameNorm +
      POSITION_WEIGHTS.WR.catch_rate.weight * catchRateNorm +
      POSITION_WEIGHTS.WR.yards_per_target.weight * yardsPerTargetNorm +
      POSITION_WEIGHTS.WR.opponent_difficulty.weight * opponentDifficulty;
  ...
  logger.info('Completed weighted score calculation for WR players', {
    ...
    seasonYear,
    week,
    wrPlayersUpdated: updates.length,
  });
}
