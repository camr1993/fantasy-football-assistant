...
/**
 * Weekly sync function for players and player stats
 * This function syncs master player data and their stats weekly for all leagues
...
Deno.serve(async (req) => {
  const timer = performance.start('weekly_sync_request');

  try {
    // Handle CORS preflight requests
    if (req.method === 'OPTIONS') {
      logger.info('Handling CORS preflight request for weekly sync');
      timer.end();
    ...
    if (req.method !== 'POST') {
      logger.warn('Invalid method for weekly sync', { method: req.method });
      timer.end();
      return new Response(
        JSON.stringify({
    ...
    logger.info('Weekly sync function triggered by cron job', {
      timestamp: new Date().toISOString(),
    });

    const syncId = crypto.randomUUID();
    logger.info('Starting weekly sync process', {
      syncId,
      timestamp: new Date().toISOString(),
    });
    ...
    if (usersError) {
      logger.error('Failed to fetch users for weekly sync', {
        error: usersError,
      });
    ...
    if (!users || users.length === 0) {
      logger.info('No users with Yahoo tokens found for weekly sync');
      timer.end();
      return new Response(
        JSON.stringify({
          success: true,
          message: 'No users available for weekly sync',
          timestamp: new Date().toISOString(),
          syncId,
        }),
        {
    ...
    logger.info('Starting master player data sync', { userId: users[0].id });

    // Sync master player data (not league-specific)
    const result = await syncMasterPlayerData(yahooToken);

    logger.info('Weekly sync process completed', {
      syncId,
      result,
      timestamp: new Date().toISOString(),
    });
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: 'Weekly sync process completed successfully',
        timestamp: new Date().toISOString(),
        syncId,
        result,
      }),
    ...
    logger.error('Error in weekly sync function', {
      error: error.message,
      stack: error.stack,
      timestamp: new Date().toISOString(),
    });
    ...
    return new Response(
      JSON.stringify({
        ...
        message: 'An error occurred during the weekly sync process',
        timestamp: new Date().toISOString(),
      }),
      {
        status: 500,
...
async function syncMasterPlayerData(yahooToken: string) {
  const syncLogId = await logSyncStart('weekly_players', null);
