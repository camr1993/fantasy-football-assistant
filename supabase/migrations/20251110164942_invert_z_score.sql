...
-- recent_std_norm now uses inverted z-score normalization: -1 * (x - mean) / std
...
BEGIN
  ...
  normalized_stats AS (
    SELECT
      ...
      -- Normalize recent_std: -1 * (x - mean) / std - inverted z-score normalization
      CASE
        WHEN recent_std_stddev > 0 THEN
          ROUND(
            (-1 * ((recent_std - recent_std_mean)::NUMERIC / recent_std_stddev))::NUMERIC,
            3
          )
        ELSE 0
...
COMMENT ON FUNCTION normalize_recent_stats_bulk IS 'Bulk normalizes recent_mean (min-max) and recent_std (inverted z-score) for all players in a league using SQL window functions, grouped by position (WR vs WR, RB vs RB, etc.)';
