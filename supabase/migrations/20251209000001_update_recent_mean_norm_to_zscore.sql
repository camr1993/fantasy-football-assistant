-- Update normalize_recent_stats_bulk to use z-score normalization for BOTH recent_mean_norm and recent_std_norm
-- Previously recent_mean_norm used min-max normalization, now using z-score: (x - mean) / std
...
BEGIN
  ...
  WITH stats_with_aggregates AS (
    SELECT
      ...
      lc.recent_std,
      p.position,
      -- Calculate mean and std per position for recent_mean (z-score normalization)
      AVG(lc.recent_mean) OVER (PARTITION BY p.position) AS recent_mean_mean,
      STDDEV(lc.recent_mean) OVER (PARTITION BY p.position) AS recent_mean_stddev,
  ...
  normalized_stats AS (
    SELECT
      player_id,
      -- Normalize recent_mean: (x - mean) / std - z-score normalization
      CASE
        WHEN recent_mean_stddev > 0 THEN
          ROUND(
            ((recent_mean - recent_mean_mean)::NUMERIC / recent_mean_stddev)::NUMERIC,
            3
          )
        ELSE 0
...
COMMENT ON FUNCTION normalize_recent_stats_bulk IS 'Bulk normalizes recent_mean and recent_std using z-score normalization for all players in a league using SQL window functions, grouped by position (WR vs WR, RB vs RB, etc.)';
