-- Create SQL function to calculate 3-week rolling averages for RB efficiency metrics
...
BEGIN
  ...
  SET
    ...
    yards_per_carry_3wk_avg = avg_data.yards_per_carry_3wk_avg,
    yards_per_target_rb_3wk_avg = avg_data.yards_per_target_rb_3wk_avg
  FROM (
    SELECT
      ps.player_id,
      ps.season_year,
      ...
      AVG(recent_weeks.yards_per_carry) FILTER (WHERE recent_weeks.yards_per_carry IS NOT NULL) AS yards_per_carry_3wk_avg,
      AVG(recent_weeks.yards_per_target_rb) FILTER (WHERE recent_weeks.yards_per_target_rb IS NOT NULL) AS yards_per_target_rb_3wk_avg
    ...
    CROSS JOIN LATERAL (
      SELECT
        ...
        ps_window.yards_per_carry,
        ps_window.yards_per_target_rb
...
COMMENT ON FUNCTION calculate_rb_efficiency_3wk_avg IS 'Calculates 3-week rolling averages for RB efficiency metrics (weighted_opportunity, touchdown_production, receiving_profile, yards_per_carry, yards_per_target_rb) for all players in a given week. Only includes weeks where the player actually played (played = true), using the most recent 3 weeks where the player played up to and including the current week.';
